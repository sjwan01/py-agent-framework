...
class TestThinkingStreamingWarning:
    """run_stream() warns once when thinking_enabled=True is actually consumed."""

    async def test_run_stream_warns_once(self, model: TestModel) -> None:
        """The first consumed run_stream() warns; a second call does not repeat."""
        warnings: list[str] = []

        def _warn(msg: str, exc: Exception | None = None) -> None:
            warnings.append(msg)

        runner = AgentRunner(model=model, system_prompt="sp", on_warning=_warn)
    ...
    async def test_run_does_not_warn(self, model: TestModel) -> None:
        """run() never warns — it is not affected by the streaming defect."""
        warnings: list[str] = []
    ...
    async def test_thinking_disabled_no_warning(self, model: TestModel) -> None:
        """thinking_enabled=False with run_stream() produces no warning."""
        warnings: list[str] = []
    ...
    async def test_warning_mentions_defect_and_workaround(
        self, model: TestModel
    ) -> None:
